...
from terminal_db.config import DbType, DbConfig, SshConfig, ConnectionProfile
...
def connect(connection_name: str) -> str:
    ...
    try:
        ...
        _db_conn.connect()
        _executor = QueryExecutor(_db_conn)
...
def execute_query(query: str) -> str:
    ...
    if not query.upper().lstrip().startswith("SELECT"):
        raise RuntimeError("Only SELECT queries are allowed for safety")
